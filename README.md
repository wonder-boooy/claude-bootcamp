...
| `--set` | `bootcamp` / `hiit` / `yoga` / `stiff` | 対話で質問（`bootcamp`） |
...
新しいセットを足す場合は、日英 2 ファイルを用意し、`src/core/verbs.ts` の `SETS` と `src/types.ts` の `VerbSet` に追記してください。いずれも「読んだ瞬間に何をすべきか分かる、回数・数字なしの具体的な動作」で揃えます。
